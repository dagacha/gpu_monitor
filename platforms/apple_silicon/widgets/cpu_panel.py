"""
CPU Panel for Apple Silicon.
Shows per-core load and frequency (from powermetrics if available).
"""
...
_BAR_W = 18
_MAX_CORES = 16  # M1 Max has 10, but leave room for future chips
...
class CPUPanel(Widget):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.border_title = "CPU Cores  (Apple Silicon)"

    def compose(self) -> ComposeResult:
        yield Static("", id="cpu-header")
        # Pre-allocate rows (two cores per row)
    ...
    def _update(self) -> None:
        ...
        self.query_one("#cpu-header", Static).update(
            "  |  ".join(parts) if parts else "CPU metrics available"
        )
