"""
Power Panel for Apple Silicon.
Shows CPU/GPU power from powermetrics and thermal pressure.
"""
...
class PowerPanel(Widget):
    ...
    def _update(self) -> None:
        ...
        if power.package_power_w is not None:
            power_parts.append(f"[bold]Total: {fmt_watts(power.package_power_w)}[/]")

        self.query_one("#power-row", Static).update(
            "  |  ".join(power_parts) if power_parts else "Power data requires sudo powermetrics"
        )

        # Thermal row
        thermal = power.thermal_pressure or "Unknown"
        thermal_text = f"Thermal Pressure: {thermal}"

        thermal_widget = self.query_one("#thermal-row", Static)

        # Apply color styling based on thermal level
        if thermal in ["Critical", "Serious"]:
            thermal_widget.set_classes("critical")
        elif thermal == "Fair":
            thermal_widget.set_classes("heavy")
        elif thermal == "Moderate":
            thermal_widget.set_classes("moderate")
        else:
